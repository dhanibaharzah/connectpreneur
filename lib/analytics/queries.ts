...
export async function getAnalyticsOverview() {
  const [row] = await sql`
    SELECT
      ...
      (SELECT COUNT(*)::int FROM analytics_events WHERE event_type IN ('whatsapp_click', 'rfq_submit')) AS whatsapp_submits,
...
export async function getMitraUniqueVisitors() {
  return sql`
    SELECT
      ...
      (
        ...
        WHERE ae.business_id = b.id
          AND ae.event_type IN ('whatsapp_click', 'rfq_submit')
      ) AS whatsapp_submits,
    ...
    LEFT JOIN analytics_events e2 ON e2.business_id = b.id
      AND e2.event_type IN ('website_click', 'social_click')
